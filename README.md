...
Argus is an intelligent web browsing and data extraction system that leverages an autonomous browsing agent and LLM-powered data extraction to collect structured information (records) from websites. It processes URLs asynchronously via a task queue, stores results in Redis, and notifies users via Telegram.
...
- **Async Task Queue**: Celery-based task processing
...
This starts the Celery worker that processes queued tasks.
...
1. **Schedule `submit.sh`** with cron to periodically submit URLs for processing
